      ...
      {ventes.length > 0 && (
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                Ils ont trouvé avec UrbanHouse360
              </h2>
              <p className="text-xl text-gray-600">
                Nos ventes récentes par quartier
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {ventes.map(vente => {
                ...
                return (
                  <div key={vente.id} className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="font-bold text-lg text-gray-900">
                          {quartier?.nom || vente.quartier}
                        </h3>
                        <p className="text-sm text-gray-600">{quartier?.secteur}</p>
                      </div>
                      <span className="px-3 py-1 bg-forest-100 text-forest-800 text-xs font-medium rounded-full">
                        {vente.type_bien}
                      </span>
                    </div>
                    ...
                    <div className="space-y-2 text-sm text-gray-700">
                      {vente.surface && (
                        <div className="flex justify-between">
                          <span>Surface :</span>
                          <span className="font-medium">{vente.surface} m²</span>
                        </div>
                      )}
                      ...
                      {vente.afficher_prix && vente.prix && (
                        <div className="flex justify-between">
                          <span>Prix :</span>
                          <span className="font-medium">{vente.prix.toLocaleString('fr-FR')} €</span>
                        </div>
                      )}

                      {vente.delai_vente_jours && (
                        <div className="flex justify-between">
                          <span>Délai de vente :</span>
                          <span className="font-medium">{vente.delai_vente_jours} jours</span>
                        </div>
                      )}
